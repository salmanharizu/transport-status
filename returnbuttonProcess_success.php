...
if ($carodostart < $cartotmil)
{
	...
	//header("location: error_odostartlessthantotal.html");
	return;
}
...
if ($carodoend <= $carodostart)
{
	...
	//header("location: error_odoendlessthanodostart.html");
	return;
}
...
if ($qtotal['total'] == '0')
{
	...
	elseif ($rowbook['car_to_service'] == '1')//car to service
	{
		...
		{
			echo "New Mileage Must More Than Old Mileage";
			//header("location: error_odoendlessthanodostart.html");
			return;
		}
	...
	$sql .= "UPDATE reference_master SET ref_seq_no = ref_seq_no + 1 WHERE ref_code in ('RK') AND ref_status ='1';";
	...
	if (mysqli_multi_query($db, $sql))  //to run the query
	{
		...
		//echo "apa niiiiii laaaa 3333333";
		...
		try
		{
			...
			$mail->isHTML(true);
			$mail->Subject = "Return The Car's Key By: $staffid-$staffname";
