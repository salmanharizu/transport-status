...
$q1 = "SELECT count(*) as total
        FROM booking_master
        WHERE staff_id = '$myusername' and book_status = '0'";
$result1 = mysqli_query($db, $q1);
$row1 = mysqli_fetch_assoc($result1);

if ($row1['total'] != '0')
{
//$result = mysqli_query($db,"SELECT * FROM booking_master WHERE staff_id = '$myusername' AND book_status = '0'");
$result = mysqli_query($db,"SELECT a.*, b.ref_desc
                            ...
                            AND book_start_date >= current_time
                            order by book_carno, book_start_date");
} else {
              ...
              <table>
                ...
                while($row = mysqli_fetch_array($result))
                {
                  ...
                  <tr class="<?php if(isset($classname)) echo $classname;?>">
                    ...
                    <td><?php
                    echo date('d-m-Y h:i:a', strtotime($row["book_start_date"]));?>
                    </td>
                    <td><?php
                    ...
                    <td><a href="editbuttonform.php?book_id=<?php echo $row["book_id"];?>&amp;
                      book_start_date=<?php echo $row["book_start_date"];?>&amp;
                      book_start_time=<?php echo $row["book_start_time"];?>&amp;
                      book_end_date=<?php echo $row["book_end_date"];?>&amp;
                      book_end_time=<?php echo $row["book_end_time"];?>&amp;
                      book_carno=<?php echo $row["book_carno"];?>&amp;
                      book_desc=<?php echo $row["book_desc"];?>">Edit</a></td>
                  </tr>
                  <?php
                  $i++;
                }
